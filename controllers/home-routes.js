...
router.get('/threads/:id/edit', withAuth, async (req, res) => {
  try {
    ...
    res.render('edit-thread', { threadData: threadData, loggedIn: req.session.loggedIn });
  } catch (err) {
    //*  Throw error if we can't connect to the server
    console.log(err);
    res.status(500).json(err);
...
router.get('/threads/:id', withAuth, async (req, res) => {
  try {
    ...
    for (let comment of comments) {
      if (comment.user_id == req.session.user_id) comment.isCommentAuthor = true;
      else comment.isCommentAuthor = false;
    }

    //*  Render the thread page with the threads data
    res.render('thread', { thread: thread, comments: comments, isThreadAuthor: isThreadAuthor, loggedIn: req.session.loggedIn });
  } catch (err) {
    //*  Throw error if we can't connect to the server
    console.log(err);
    res.status(500).json(err);
